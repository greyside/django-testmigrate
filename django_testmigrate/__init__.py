...
class TestMigrateCommand(migrate.Command):
    def migration_progress_callback(self, action, migration, *args, **kwargs):
        ...
        if action in ('apply_success', 'unapply_success',) and hasattr(migration, 'test'):
            if self.verbosity > 0:
                self.stdout.write('Running test for %s...' % migration.name, self.style.MIGRATE_SUCCESS, ending='')
            
            project_state = executor.loader.project_state((migration.app_label, migration.name), at_end=False)
            migration.test(project_state)
